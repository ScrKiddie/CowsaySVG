...
func (r *Renderer) calculateDimensions(lines []string) (width, height int, maxLineCharLength int) {
	maxLen := 0
	...
	if maxLen == 0 {
		return 0, calculatedHeight, 0
	}
	paddingBottom := int(float64(r.config.FontSize) * 0.35)
	if paddingBottom == 0 && r.config.FontSize > 0 {
		paddingBottom = 2
	}
	return maxLen * r.config.CharWidth, calculatedHeight + paddingBottom, maxLen
}

func (r *Renderer) renderStaticTextWithGradient(lines []string, colors []string, maxLineCharLen int) {
	if len(colors) == 0 {
		...
		for charIdx, char := range line {
			...
			if len(colors) == 1 {
				...
				if maxLineCharLen <= 1 {
					...
					denominator := float64(maxLineCharLen - 1)
					colorIndex = int(math.Round(float64(charIdx) * float64(len(colors)-1) / denominator))
				}
...
func (r *Renderer) Render(text string, params AnimationParams, isStatic bool) {
	...
	width, height, maxLineCharLen := r.calculateDimensions(lines)

	r.Start(width, height)
	defer r.End()

	if isStatic {
		r.renderStaticTextWithGradient(lines, params.Colors, maxLineCharLen)
		return
	}
	...
	if numColors == 0 {
		r.renderStaticTextWithGradient(lines, []string{}, maxLineCharLen)
		return
	}
	...
	for lineIdx, line := range lines {
		...
		for charIdx, char := range line {
			x := r.config.CharWidth * charIdx
			delay := CalculateAnimationDelay(params, lineIdx, charIdx, numberOfLines, numberOfCharsInLine, maxLineCharLen)
